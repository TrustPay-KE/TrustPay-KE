-- TrustPay KE - Perfect Database Setup (No Recursion Issues)
-- Clean, simple, and working RLS policies
...
-- PERFECT RLS POLICIES (No Recursion)
...
-- Escrows table - Simple and clean
...
-- Transactions table - Simple and clean
...
-- Disputes table - Simple and clean
...
CREATE POLICY "Users can view disputes they are involved in" ON disputes
    FOR SELECT USING (raised_by = auth.uid() OR 
        EXISTS (SELECT 1 FROM escrows WHERE id = disputes.escrow_id AND (buyer_id = auth.uid() OR seller_id = auth.uid())));
...
-- Notifications table - Simple and clean
...
-- Invites table - Simple and clean
...
CREATE POLICY "Users can view invites they created or received" ON invites
    FOR SELECT USING (invited_by = auth.uid() OR 
        (EXISTS (SELECT 1 FROM users WHERE id = auth.uid() AND email = invites.email)) OR
        (EXISTS (SELECT 1 FROM users WHERE id = auth.uid() AND phone = invites.phone)));
...
BEGIN
    RETURN QUERY
    SELECT 'users'::text, 'OK'::text WHERE EXISTS (SELECT 1 FROM users LIMIT 1) OR NOT EXISTS (SELECT 1 FROM users LIMIT 1)
    UNION ALL
    SELECT 'escrows'::text, 'OK'::text WHERE EXISTS (SELECT 1 FROM escrows LIMIT 1) OR NOT EXISTS (SELECT 1 FROM escrows LIMIT 1)
    UNION ALL
    SELECT 'transactions'::text, 'OK'::text WHERE EXISTS (SELECT 1 FROM transactions LIMIT 1) OR NOT EXISTS (SELECT 1 FROM transactions LIMIT 1)
    UNION ALL
    SELECT 'disputes'::text, 'OK'::text WHERE EXISTS (SELECT 1 FROM disputes LIMIT 1) OR NOT EXISTS (SELECT 1 FROM disputes LIMIT 1)
    UNION ALL
    SELECT 'kyc'::text, 'OK'::text WHERE EXISTS (SELECT 1 FROM kyc LIMIT 1) OR NOT EXISTS (SELECT 1 FROM kyc LIMIT 1)
    UNION ALL
    SELECT 'notifications'::text, 'OK'::text WHERE EXISTS (SELECT 1 FROM notifications LIMIT 1) OR NOT EXISTS (SELECT 1 FROM notifications LIMIT 1)
    UNION ALL
    SELECT 'invites'::text, 'OK'::text WHERE EXISTS (SELECT 1 FROM invites LIMIT 1) OR NOT EXISTS (SELECT 1 FROM invites LIMIT 1);
END;
$$ LANGUAGE plpgsql;
